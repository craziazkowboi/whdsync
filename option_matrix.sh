...
for s in AGA ECS RTG AGA_Laced ECS_Laced CD32 art; do mkdir -p "$ROOT/iGame_$s/Covers/Games/A/Alpha"; echo "$s" > "$ROOT/iGame_$s/Covers/Games/A/Alpha/iGame.iff"; done
mkdir -p "$ROOT/TinyLauncher/Game"
...
for s in all.sh start.sh extract.sh merge.sh sort.sh update.sh quick.sh doctor.sh uninstall_deps.sh setup.sh; do
    tt "0" "$s --help" ./"$s" --help
...
for a in --update --extract --merge --sort --quick; do tt "0|2" "start.sh $a" ./start.sh $a; done
